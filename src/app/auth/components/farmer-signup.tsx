...
export function FarmerSignUpForm({ className, ...props }: UserAuthFormProps) {
  ...
  const [ setAuth, setLogin]=useStore((store)=> [store.login ,store.setLoginForm])
  const router=useRouter()

  const formSchema=z.object({
   
    location: z.string().min(1, { message: 'Location Required' }),
    phone: z.string().regex(/^(?:\+254|0)[17]\d{8}$/,  { message: 'Enter a valid phone number' }),
 ...
 type Values= z.infer<typeof formSchema>;
  const {handleSubmit, setValue, formState:{errors}, register}=useForm<Values>({
    resolver: zodResolver(formSchema),
  })
   const onSubmit=( data: Values)=> {setAuth(), console.log(data); router.replace('/dashboard')}
...
<ErrorMessage
    ...
    <Button variant={'ghost'} className="text-primary"  onClick={  ()=> {setLogin();  router.replace('/auth')}}>
Login
    </Button>
    </div>
  )
